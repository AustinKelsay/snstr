...
async function main() {
    try {
        ...
        if (USE_EPHEMERAL) {
            ...
            ephemeralRelay = new ephemeral_relay_1.NostrRelay(3002, 60); // Purge every 60 seconds
            await ephemeralRelay.start();
            console.log('Ephemeral relay started');
            // Initialize clients with the ephemeral relay
            alice = new src_1.Nostr([ephemeralRelay.url]);
            bob = new src_1.Nostr([ephemeralRelay.url]);
        }
        else {
            // Use public relays
            const PUBLIC_RELAYS = ['wss://relay.primal.net', 'wss://relay.nostr.band'];
            console.log(`Using public relays: ${PUBLIC_RELAYS.join(', ')}`);
            alice = new src_1.Nostr(PUBLIC_RELAYS);
            bob = new src_1.Nostr(PUBLIC_RELAYS);
        }
        ...
        bob.subscribe([{ kinds: [4], '#p': [bobKeys.publicKey] }], (event, relay) => {
            try {
        ...
        const dmEvent = await alice.publishDirectMessage(message, bobKeys.publicKey);
        if (dmEvent) {
