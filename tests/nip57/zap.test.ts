...
jest.mock('../../src/utils/crypto', () => {
  ...
  return {
    ...
    verifySignature: jest.fn().mockImplementation(originalModule.verifySignature)
  };
});
...
jest.mock('../../src/nip57/utils', () => {
  ...
  return {
    ...originalModule,
    parseBolt11Invoice: jest.fn().mockImplementation(originalModule.parseBolt11Invoice),
    fetchLnurlPayMetadata: jest.fn().mockImplementation(originalModule.fetchLnurlPayMetadata)
  };
});

// Import mocked functions after mocking
import { sha256, verifySignature } from '../../src/utils/crypto';
import { parseBolt11Invoice, fetchLnurlPayMetadata } from '../../src/nip57/utils';

// Store originals for restoration
const originalSha256 = jest.requireActual('../../src/utils/crypto').sha256;
const originalVerifySignature = jest.requireActual('../../src/utils/crypto').verifySignature;
const originalParseBolt11Invoice = jest.requireActual('../../src/nip57/utils').parseBolt11Invoice;
const originalFetchLnurlPayMetadata = jest.requireActual('../../src/nip57/utils').fetchLnurlPayMetadata;
...
describe('NIP-57: Lightning Zaps', () => {
  ...
  beforeEach(() => {
    ...
    (parseBolt11Invoice as jest.Mock).mockImplementation(originalParseBolt11Invoice);
    (fetchLnurlPayMetadata as jest.Mock).mockImplementation(originalFetchLnurlPayMetadata);
  });
    ...
    describe('Description Hash Validation', () => {
      ...
      it('should work with a real bolt11 invoice', async () => {
        ...
        const mockInvoiceData = {
          ...
          descriptionHash: "5d006d2cf1e73c7148e7519a4c68adc81642ce0e25a432b2434c99f97344c15f"
        };
        ...
        const descHashBytes = new Uint8Array(
          mockInvoiceData.descriptionHash.match(/.{1,2}/g)!.map(byte => parseInt(byte, 16))
        );
        ...
        const validationWithWrongHash = validateZapReceipt(signedZapReceipt, lnurlServerKeypair.publicKey);
        expect(validationWithWrongHash.valid).toBe(false);
        expect(validationWithWrongHash.message).toContain('Description hash mismatch');
      });
    });
  });
  ...
  describe('Integration: Full Zap Flow', () => {
    ...
    it('should complete the full zap flow with validation', async () => {
      ...
      const zapRequestTemplate = createZapRequest({
        ...
        relays: ['wss://relay.example.com'],
        content: 'Great post!',
        lnurl: 'lnurl1dp68gurn8ghj7um9wfmxjcm99e3k7mf0wvhxjmmrv9kzmmrv9kzumrfde9jq' // fake LNURL
      }, senderKeypair.publicKey);
      ...
      const zapReceiptTemplate = createZapReceipt({
        ...
        bolt11: realInvoice,
        zapRequest: signedZapRequest,
        preimage: '5d006d2cf1e73c7148e7519a4c68adc81642ce0e25a432b2434c99f97344c15f' // random preimage
      }, lnurlServerKeypair.publicKey);
