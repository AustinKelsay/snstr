...
describe('NIP-07 Core Functions', () => {
  ...
  describe('getPublicKey', () => {
    ...
    it('should throw an error if NIP-07 is not supported', async () => {
      delete (global as any).window.nostr;
      await expect(nip07.getPublicKey()).rejects.toThrow('NIP-07 extension not available');
    });
    ...
    it('should throw an error if the extension call fails', async () => {
      ...
      await expect(nip07.getPublicKey()).rejects.toThrow('Failed to get public key');
    });
  });
  ...
  describe('signEvent', () => {
    ...
    it('should throw an error if NIP-07 is not supported', async () => {
      delete (global as any).window.nostr;
      await expect(nip07.signEvent(mockEvent)).rejects.toThrow('NIP-07 extension not available');
    });
    
    it('should throw an error if the extension call fails', async () => {
      mockSignEvent.mockRejectedValueOnce(new Error('Extension error'));
      await expect(nip07.signEvent(mockEvent)).rejects.toThrow('Failed to sign event');
    });
  });
  ...
  describe('encryptNip04', () => {
    it('should call window.nostr.nip04.encrypt and return the encrypted message', async () => {
      ...
      expect(mockEncryptNip04).toHaveBeenCalledWith('recipient-pubkey', 'plaintext');
    });
    
    it('should throw an error if NIP-07 is not supported', async () => {
      delete (global as any).window.nostr;
      await expect(nip07.encryptNip04('pubkey', 'plaintext')).rejects.toThrow('NIP-07 extension not available');
    });
    
    it('should throw an error if nip04 encryption is not supported', async () => {
      delete (global as any).window.nostr.nip04;
      await expect(nip07.encryptNip04('pubkey', 'plaintext')).rejects.toThrow('NIP-04 encryption not supported');
    });
    
    it('should throw an error if the extension call fails', async () => {
      mockEncryptNip04.mockRejectedValueOnce(new Error('Extension error'));
      await expect(nip07.encryptNip04('pubkey', 'plaintext')).rejects.toThrow('Failed to encrypt message');
    });
  });
  ...
  describe('decryptNip04', () => {
    it('should call window.nostr.nip04.decrypt and return the decrypted message', async () => {
      ...
      expect(mockDecryptNip04).toHaveBeenCalledWith('sender-pubkey', 'ciphertext');
    });
    
    it('should throw an error if NIP-07 is not supported', async () => {
      delete (global as any).window.nostr;
      await expect(nip07.decryptNip04('pubkey', 'ciphertext')).rejects.toThrow('NIP-07 extension not available');
    });
    
    it('should throw an error if nip04 decryption is not supported', async () => {
      delete (global as any).window.nostr.nip04;
      await expect(nip07.decryptNip04('pubkey', 'ciphertext')).rejects.toThrow('NIP-04 decryption not supported');
    });
    
    it('should throw an error if the extension call fails', async () => {
      mockDecryptNip04.mockRejectedValueOnce(new Error('Extension error'));
      await expect(nip07.decryptNip04('pubkey', 'ciphertext')).rejects.toThrow('Failed to decrypt message');
    });
  });
  ...
  describe('encryptNip44', () => {
    it('should call window.nostr.nip44.encrypt and return the encrypted message', async () => {
      ...
      expect(mockEncryptNip44).toHaveBeenCalledWith('recipient-pubkey', 'plaintext');
    });
    
    it('should throw an error if NIP-07 is not supported', async () => {
      delete (global as any).window.nostr;
      await expect(nip07.encryptNip44('pubkey', 'plaintext')).rejects.toThrow('NIP-07 extension not available');
    });
    
    it('should throw an error if nip44 encryption is not supported', async () => {
      delete (global as any).window.nostr.nip44;
      await expect(nip07.encryptNip44('pubkey', 'plaintext')).rejects.toThrow('NIP-44 encryption not supported');
    });
    
    it('should throw an error if the extension call fails', async () => {
      mockEncryptNip44.mockRejectedValueOnce(new Error('Extension error'));
      await expect(nip07.encryptNip44('pubkey', 'plaintext')).rejects.toThrow('Failed to encrypt message');
    });
  });
  ...
  describe('decryptNip44', () => {
    it('should call window.nostr.nip44.decrypt and return the decrypted message', async () => {
      ...
      expect(mockDecryptNip44).toHaveBeenCalledWith('sender-pubkey', 'ciphertext');
    });
    
    it('should throw an error if NIP-07 is not supported', async () => {
      delete (global as any).window.nostr;
      await expect(nip07.decryptNip44('pubkey', 'ciphertext')).rejects.toThrow('NIP-07 extension not available');
    });
    
    it('should throw an error if nip44 decryption is not supported', async () => {
      delete (global as any).window.nostr.nip44;
      await expect(nip07.decryptNip44('pubkey', 'ciphertext')).rejects.toThrow('NIP-44 decryption not supported');
    });
    
    it('should throw an error if the extension call fails', async () => {
      mockDecryptNip44.mockRejectedValueOnce(new Error('Extension error'));
      await expect(nip07.decryptNip44('pubkey', 'ciphertext')).rejects.toThrow('Failed to decrypt message');
    });
  });
}); 