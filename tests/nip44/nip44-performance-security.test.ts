...
describe("NIP-44 Performance Tests", () => {
  ...
  describe("Encryption/Decryption Performance", () => {
    ...
    test("should handle large messages efficiently", () => {
      ...
      console.log(
        `Large message avg encryption time: ${avgEncryptTime.toFixed(2)}ms`,
      );
      console.log(
        `Large message avg decryption time: ${avgDecryptTime.toFixed(2)}ms`,
      );
    ...
    test("should perform key derivation efficiently", () => {
      ...
      if (avgTime > 20) {
        console.warn(
          `Key derivation is slower than expected (${avgTime.toFixed(2)}ms per operation). This may indicate system load or slower hardware.`,
        );
      }
    });
    ...
    test("should perform message key derivation efficiently", () => {
      ...
      const avgTime = totalTime / iterations;
      console.log(
        `Average message key derivation time: ${avgTime.toFixed(2)}ms`,
      );
  ...
  describe("Throughput Tests", () => {
    test("should maintain consistent performance under load", () => {
      ...
      console.log(
        `Throughput stats - Avg: ${avgTime.toFixed(2)}ms, Min: ${minTime.toFixed(2)}ms, Max: ${maxTime.toFixed(2)}ms, StdDev: ${stdDev.toFixed(2)}ms`,
      );
      ...
      if (stdDev > avgTime * 1.0) {
        console.warn(
          `High performance variance detected (StdDev: ${stdDev.toFixed(2)}ms vs Avg: ${avgTime.toFixed(2)}ms) - this may indicate system load`,
        );
      }
      if (maxTime > avgTime * 10) {
        console.warn(
          `High maximum time detected (Max: ${maxTime.toFixed(2)}ms vs Avg: ${avgTime.toFixed(2)}ms) - this may indicate system load`,
        );
      }
    });
  });
...
describe("NIP-44 Security Tests", () => {
  describe("Constant-Time Comparison Security", () => {
    test("should have consistent timing for equal arrays", () => {
      ...
      console.log(
        `Equal arrays timing - Avg: ${avgTime.toFixed(4)}ms, Min: ${minTime.toFixed(4)}ms, Max: ${maxTime.toFixed(4)}ms`,
      );
    ...
    test("should have consistent timing for different arrays", () => {
      ...
      console.log(
        `Different arrays timing - Avg: ${avgTime.toFixed(4)}ms, Min: ${minTime.toFixed(4)}ms, Max: ${maxTime.toFixed(4)}ms`,
      );
    ...
    test("should handle timing attack scenarios", () => {
      ...
      const avgFirstTime =
        firstByteTimes.reduce((a, b) => a + b, 0) / iterations;
      const avgLastTime = lastByteTimes.reduce((a, b) => a + b, 0) / iterations;

      console.log(
        `First byte diff avg: ${avgFirstTime.toFixed(4)}ms, Last byte diff avg: ${avgLastTime.toFixed(4)}ms`,
      );
