...
describe('NIP-44 Official Compatibility Tests', () => {
  ...
  describe('Conversation Key Derivation', () => {
    ...
    test('should be symmetric (key derivation works in both directions)', () => {
      for (const vector of vectors) {
        ...
        expect(Buffer.from(key1to2).toString('hex')).toBe(Buffer.from(key2to1).toString('hex'));
      }
    });
  });
  
  describe('Message Key Derivation', () => {
    const conversationKeyHex = testVectors.v2.valid.get_message_keys.conversation_key;
    ...
    test('should derive correct message keys for all test vectors', () => {
      ...
      for (const vector of keyVectors) {
        ...
        expect(Buffer.from(keys.chacha_nonce).toString('hex')).toBe(chacha_nonce);
  ...
  describe('Complete Encrypt/Decrypt Flow', () => {
    ...
    test('should correctly encrypt with known values to produce expected output', () => {
      ...
      for (let i = 0; i < 5 && i < vectors.length; i++) {
        ...
        const { sec1, sec2, nonce: nonceHex, plaintext, payload: expectedPayload } = vector;
    ...
    test('should correctly decrypt official test vector payloads', () => {
      for (const vector of vectors) {
        ...
        try {
          ...
          fail(`Failed to decrypt test vector with plaintext "${plaintext}": ${error}`);
        }
      }
    });
  ...
  describe('Edge Cases', () => {
    test('should handle minimal (1 byte) and maximal (65535 bytes) message sizes', () => {
      ...
      expect(decrypt(encrypt(message31, sec1, pub2), sec2, pub1)).toBe(message31);
      expect(decrypt(encrypt(message32, sec1, pub2), sec2, pub1)).toBe(message32);
      expect(decrypt(encrypt(message33, sec1, pub2), sec2, pub1)).toBe(message33);
