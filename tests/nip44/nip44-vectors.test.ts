...
describe("NIP-44 Test Vectors", () => {
  ...
  describe("Message Encryption Roundtrip", () => {
    ...
    const testMessages = [
      ...
      {
        name: "JSON data",
        message: JSON.stringify({ test: "value", nested: { num: 42 } }),
      },
    ];
  ...
  describe("Padding Scheme", () => {
    ...
    test("should pad small messages to a minimum size", () => {
      ...
      expect(
        Math.abs(encrypted1.length - encrypted2.length),
      ).toBeLessThanOrEqual(4);

      // Verify decryption still works
      expect(
        decrypt(encrypted1, bobKeypair.privateKey, aliceKeypair.publicKey),
      ).toBe(oneChar);
      expect(
        decrypt(encrypted2, bobKeypair.privateKey, aliceKeypair.publicKey),
      ).toBe(twoChars);
    });
    ...
    test("should handle exact minimum (1 byte) and maximum (65535 bytes) message sizes per NIP-44 spec", () => {
      ...
      const encryptedMin = encrypt(
        minMessage,
        aliceKeypair.privateKey,
        bobKeypair.publicKey,
      );
      const decryptedMin = decrypt(
        encryptedMin,
        bobKeypair.privateKey,
        aliceKeypair.publicKey,
      );
      ...
      const encryptedMax = encrypt(
        maxMessage,
        aliceKeypair.privateKey,
        bobKeypair.publicKey,
      );
      const decryptedMax = decrypt(
        encryptedMax,
        bobKeypair.privateKey,
        aliceKeypair.publicKey,
      );
