...
describe("NIP-66", () => {
  ...
  describe("Type alias behavior and data transformation", () => {
    test("supportedNips should accept both numbers and strings (NipNumber type)", () => {
      ...
      expect(() =>
        createRelayDiscoveryEvent(options, validPubkey),
      ).not.toThrow();
  ...
  describe("Validation for createRelayDiscoveryEvent", () => {
    test("should throw error for missing options", () => {
      expect(() =>
        createRelayDiscoveryEvent(
          null as unknown as RelayDiscoveryEventOptions,
          validPubkey,
        ),
      ).toThrow("Options object is required");
      expect(() =>
        createRelayDiscoveryEvent(
          undefined as unknown as RelayDiscoveryEventOptions,
          validPubkey,
        ),
      ).toThrow("Options object is required");
    });
    ...
    test("should throw error for invalid pubkey", () => {
      ...
      expect(() => createRelayDiscoveryEvent(options, "")).toThrow(
        "Valid pubkey is required",
      );
      expect(() => createRelayDiscoveryEvent(options, "   ")).toThrow(
        "Valid pubkey is required",
      );
      expect(() =>
        createRelayDiscoveryEvent(options, null as unknown as string),
      ).toThrow("Valid pubkey is required");
      expect(() =>
        createRelayDiscoveryEvent(options, undefined as unknown as string),
      ).toThrow("Valid pubkey is required");
    });

    test("should throw error for invalid relay URL", () => {
      expect(() =>
        createRelayDiscoveryEvent({ relay: "" }, validPubkey),
      ...
      expect(() =>
        createRelayDiscoveryEvent(
          {
            relay: null,
          } as TestRelayDiscoveryOptions as RelayDiscoveryEventOptions,
          validPubkey,
        ),
      ).toThrow("Valid relay URL is required");
      expect(() =>
        createRelayDiscoveryEvent({ relay: "http://example.com" }, validPubkey),
      ).toThrow(
        /Invalid relay URL scheme.*Relay URLs must use WebSocket protocols/,
      );
      expect(() =>
        createRelayDiscoveryEvent({ relay: "wss://invalid url" }, validPubkey),
      ).toThrow();
    });

    test("should throw error for invalid RTT values", () => {
      const baseOptions: RelayDiscoveryEventOptions = {
        relay: "wss://relay.example.com",
      };

      expect(() =>
        createRelayDiscoveryEvent({ ...baseOptions, rttOpen: -1 }, validPubkey),
      ).toThrow("rttOpen must be a non-negative number");
      expect(() =>
        createRelayDiscoveryEvent(
          {
            ...baseOptions,
      ...
      expect(() =>
        createRelayDiscoveryEvent(
          { ...baseOptions, rttOpen: Infinity },
          validPubkey,
        ),
      ).toThrow("rttOpen must be a non-negative number");
      expect(() =>
        createRelayDiscoveryEvent({ ...baseOptions, rttRead: -5 }, validPubkey),
      ).toThrow("rttRead must be a non-negative number");
      expect(() =>
        createRelayDiscoveryEvent(
          { ...baseOptions, rttWrite: NaN },
          validPubkey,
        ),
      ).toThrow("rttWrite must be a non-negative number");
    });
    ...
    test("should throw error for invalid array fields", () => {
      ...
      expect(() =>
        createRelayDiscoveryEvent(
          {
            ...baseOptions,
            supportedNips: "not array",
          } as TestRelayDiscoveryOptions as RelayDiscoveryEventOptions,
          validPubkey,
        ),
      ).toThrow("supportedNips must be an array");
      expect(() =>
        createRelayDiscoveryEvent(
          {
            ...baseOptions,
      ...
      ).toThrow("kinds must be an array");
    });
    ...
    test("should throw error for invalid geohash", () => {
      ...
      expect(() =>
        createRelayDiscoveryEvent({ ...baseOptions, geohash: "" }, validPubkey),
      ).toThrow("geohash must be a non-empty string");
      expect(() =>
        createRelayDiscoveryEvent(
          { ...baseOptions, geohash: "   " },
          validPubkey,
      ...
      expect(() =>
        createRelayDiscoveryEvent(
          { ...baseOptions, geohash: "invalid-chars!" },
          validPubkey,
        ),
      ).toThrow("geohash must be alphanumeric and at most 12 characters");
      expect(() =>
        createRelayDiscoveryEvent(
          { ...baseOptions, geohash: "verylonggeohashvalue" },
          validPubkey,
        ),
      ).toThrow("geohash must be alphanumeric and at most 12 characters");
    });
    ...
    test("should throw error for invalid supportedNips values", () => {
      ...
      expect(() =>
        createRelayDiscoveryEvent(
          { ...baseOptions, supportedNips: [-1] },
          validPubkey,
        ),
      ).toThrow("supportedNips must contain valid positive integers");
      expect(() =>
        createRelayDiscoveryEvent(
          { ...baseOptions, supportedNips: [1.5] },
          validPubkey,
        ),
      ).toThrow("supportedNips must contain valid positive integers");
      expect(() =>
        createRelayDiscoveryEvent(
          { ...baseOptions, supportedNips: ["invalid"] },
          validPubkey,
        ),
      ).toThrow(
      ...
      ).toThrow("supportedNips must contain numbers or strings");
    });
    ...
    test("should throw error for invalid kinds values", () => {
      ...
      expect(() =>
        createRelayDiscoveryEvent({ ...baseOptions, kinds: [-1] }, validPubkey),
      ).toThrow("kinds must contain valid non-negative integers");
      expect(() =>
        createRelayDiscoveryEvent(
          { ...baseOptions, kinds: [1.5] },
          validPubkey,
        ),
      ).toThrow("kinds must contain valid non-negative integers");
      expect(() =>
      ...
      ).toThrow("kinds must contain numbers or strings");
    });
    ...
    test("should throw error for invalid string arrays", () => {
      ...
      ).toThrow("requirements must contain non-empty strings");
      expect(() =>
        createRelayDiscoveryEvent(
          {
            ...baseOptions,
      ...
      ).toThrow("topics must contain non-empty strings");
    });
    ...
    test("should throw error for invalid additionalTags", () => {
      ...
      expect(() =>
        createRelayDiscoveryEvent(
          {
            ...baseOptions,
            additionalTags: ["not array"],
          } as TestRelayDiscoveryOptions as RelayDiscoveryEventOptions,
          validPubkey,
        ),
      ).toThrow("additionalTags must be an array of string arrays");
      expect(() =>
        createRelayDiscoveryEvent(
          {
            ...baseOptions,
      ...
      ).toThrow("additionalTags must contain arrays of strings");
    });
    ...
    test("should accept valid options", () => {
      ...
      expect(() =>
        createRelayDiscoveryEvent(validOptions, validPubkey),
      ).not.toThrow();
    });
  });
  ...
  describe("Validation for createRelayMonitorAnnouncement", () => {
    test("should throw error for missing options", () => {
      expect(() =>
        createRelayMonitorAnnouncement(
          null as unknown as RelayMonitorAnnouncementOptions,
          validPubkey,
        ),
      ).toThrow("Options object is required");
      expect(() =>
        createRelayMonitorAnnouncement(
          undefined as unknown as RelayMonitorAnnouncementOptions,
          validPubkey,
        ),
      ).toThrow("Options object is required");
    });
    ...
    test("should throw error for invalid pubkey", () => {
      ...
      expect(() => createRelayMonitorAnnouncement(options, "")).toThrow(
        "Valid pubkey is required",
      );
      expect(() =>
        createRelayMonitorAnnouncement(options, null as unknown as string),
      ).toThrow("Valid pubkey is required");
    });
    ...
    test("should throw error for invalid frequency", () => {
      expect(() =>
        createRelayMonitorAnnouncement(
          {
            frequency: undefined,
          } as TestRelayMonitorOptions as RelayMonitorAnnouncementOptions,
          validPubkey,
        ),
      ...
      expect(() =>
        createRelayMonitorAnnouncement({ frequency: 0 }, validPubkey),
      ).toThrow("frequency must be a positive integer");
      expect(() =>
        createRelayMonitorAnnouncement({ frequency: -1 }, validPubkey),
      ).toThrow("frequency must be a positive integer");
      expect(() =>
        createRelayMonitorAnnouncement({ frequency: 1.5 }, validPubkey),
      ).toThrow("frequency must be a positive integer");
      expect(() =>
        createRelayMonitorAnnouncement(
          {
    ...
    test("should throw error for invalid timeouts", () => {
      ...
      ).toThrow("timeouts must be an array");
      expect(() =>
        createRelayMonitorAnnouncement(
          {
            ...baseOptions,
      ...
      ).toThrow("timeouts must contain timeout definition objects");
      expect(() =>
        createRelayMonitorAnnouncement(
          {
            ...baseOptions,
      ...
      ).toThrow("timeout value is required");
      expect(() =>
        createRelayMonitorAnnouncement(
          { ...baseOptions, timeouts: [{ value: 0 }] },
          validPubkey,
        ),
      ).toThrow("timeout value must be a positive integer");
      expect(() =>
        createRelayMonitorAnnouncement(
          { ...baseOptions, timeouts: [{ value: -1 }] },
          validPubkey,
        ),
      ).toThrow("timeout value must be a positive integer");
      expect(() =>
        createRelayMonitorAnnouncement(
          { ...baseOptions, timeouts: [{ value: 1.5 }] },
          validPubkey,
        ),
      ).toThrow("timeout value must be a positive integer");
      expect(() =>
        createRelayMonitorAnnouncement(
          { ...baseOptions, timeouts: [{ value: 5000, test: "" }] },
          validPubkey,
        ),
      ).toThrow("timeout test must be a non-empty string if provided");
    });
    ...
    test("should throw error for invalid checks", () => {
      ...
      ).toThrow("checks must contain non-empty strings");
      expect(() =>
        createRelayMonitorAnnouncement(
          {
            ...baseOptions,
    ...
    test("should accept valid options", () => {
      const validOptions: RelayMonitorAnnouncementOptions = {
        frequency: 3600,
        timeouts: [{ value: 5000, test: "connect" }, { value: 3000 }],
        checks: ["ws", "nip11"],
        geohash: "9q8yy",
      ...
      expect(() =>
        createRelayMonitorAnnouncement(validOptions, validPubkey),
      ).not.toThrow();
    });
  });
