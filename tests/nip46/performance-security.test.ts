...
describe("NIP-46 Performance & DoS Protection", () => {
  ...
  describe("Resource Exhaustion Protection", () => {
    ...
    test("limits number of concurrent requests", async () => {
      ...
      const requests = Array(5)
        .fill(null)
        .map(() => client.ping()); // Reduced from 10
    ...
    test("handles large tag arrays without memory exhaustion", async () => {
      ...
      const largeTags = Array(50)
        .fill(null)
        .map((_, i) => ["t", `tag${i}`]); // Reduced from 100
  ...
  describe("Connection Handling", () => {
    test("handles rapid connection attempts", async () => {
      ...
      const connectionPromises = Array(3)
        .fill(null)
        .map(async () => {
          // Reduced from 5
          const tempClient = new SimpleNIP46Client([relay.url], {
            timeout: 2000,
          }); // Reduced timeout
          try {
    ...
    test("cleans up resources on connection failure", async () => {
      ...
      const invalidClient = new SimpleNIP46Client([relay.url], {
        timeout: 1000,
      });
