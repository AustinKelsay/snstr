...
describe("NIP-46 Input Validation Security", () => {
  ...
  describe("Connection String Validation", () => {
    ...
    test("validates connection string length", () => {
      ...
      expect(() => parseConnectionString(longString)).toThrow(
        NIP46SecurityError,
      );
    });
    ...
    test("validates secret token length", () => {
      ...
      const validConnection = parseConnectionString(
        `bunker://${validPubkey}?secret=validSecret123`,
      );
    ...
    test("validates permissions format", () => {
      ...
      expect(validConnection.permissions).toEqual([
        "sign_event:1",
        "get_public_key",
        "ping",
      ]);
      ...
      expect(mixedConnection.permissions).toEqual([
        "sign_event:1",
        "get_public_key",
      ]);
    });
    ...
    test("comprehensive security pattern validation", () => {
      ...
      const securityPatterns = [
        ...
        {
          pattern: "https://example.com://evil.com",
        ...
        {
          pattern: "<link rel=stylesheet href=data:,*{x:expression(alert(1))}>",
          name: "link XSS",
        },
      ];
      ...
      securityPatterns.forEach(({ pattern }) => {
        ...
        expect(() => parseConnectionString(maliciousString)).toThrow(
          NIP46SecurityError,
        );
        ...
        expect(() => parseConnectionString(maliciousQuery)).toThrow(
          NIP46SecurityError,
        );
      });
    });
    ...
    test("validates query parameter structure", () => {
      ...
      expect(() => {
        parseConnectionString(
          `bunker://${validPubkey}?relay=wss://relay.com?evil=param`,
        );
      }).toThrow(NIP46SecurityError);

      // Valid single query parameter should work
      const validConnection = parseConnectionString(
        `bunker://${validPubkey}?relay=wss://relay.com`,
      );
  ...
  describe("Rate Limiting", () => {
    ...
    test("prevents DoS with large numbers of simultaneous requests", async () => {
      ...
      const requests = Array(10)
        .fill(null)
        .map(() => client.ping()); // Reduced from 20
  ...
  describe("Error Handling Security", () => {
    test("does not leak sensitive information in errors", async () => {
      ...
      try {
        ...
        throw new Error(
          "Expected client.connect to throw an error, but it succeeded",
        );
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : String(error);
    ...
    test("handles encryption errors securely", async () => {
      ...
      try {
        ...
        // If we reach this point, the test should fail
        throw new Error(
          "Expected nip44Encrypt to throw an error, but it succeeded",
        );
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : String(error);
  ...
  describe("Auth URL Domain Whitelist Validation", () => {
    test("validates auth URLs against domain whitelist", () => {
      ...
      const isValidAuthUrl = (
        clientWithWhitelist as unknown as {
          isValidAuthUrl: (url: string) => boolean;
        }
      )["isValidAuthUrl"].bind(clientWithWhitelist);
      ...
      expect(isValidAuthUrl("https://secure.auth.example.com/callback")).toBe(
        true,
      );
    ...
    test("allows all valid HTTPS URLs when no whitelist is configured", () => {
      ...
      const isValidAuthUrl = (
        clientWithoutWhitelist as unknown as {
          isValidAuthUrl: (url: string) => boolean;
        }
      )["isValidAuthUrl"].bind(clientWithoutWhitelist);
    ...
    test("validates against all other security checks with whitelist", () => {
      ...
      expect(isValidAuthUrl("https://trusted-domain.com/auth<script>")).toBe(
        false,
      );
