import { NostrEvent, Filter, NostrKind } from '../../src/types/nostr';
import { NostrRelay } from '../../src/utils/ephemeral-relay';
import { createSignedEvent, createEvent, UnsignedEvent, validateEvent, NostrValidationError } from '../../src/nip01/event';
  ...
  describe('Relay-dependent NIP-02 features', () => {
    ...
    it('Fetching Follows: should fetch the list of pubkeys a user follows', async () => {
      ...
      const contactListEvent = await createSignedEvent({
        ...
        pubkey: userAPubKey,
        created_at: Math.floor(Date.now() / 1000),
        tags: followedPubkeys.map(pk => ['p', pk, RELAY_URL, `User_${pk.substring(0,3)}`]),
        content: '',
      }, userAPrivKey);
      ...
      await new Promise<void>(resolve => {
        const onEoseSub = client.subscribe(
            [{ kinds: [NostrKind.Contacts], authors: [userAPubKey], limit: 1 }], 
            () => {}, 
            () => { client.unsubscribe(onEoseSub); resolve(); }
        );
        setTimeout(() => { client.unsubscribe(onEoseSub); resolve();}, 3000); // Timeout
      });
