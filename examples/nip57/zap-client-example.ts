...
import { getLightningAddressUrl, fetchLnurlPayMetadata, buildZapCallbackUrl } from '../../src/nip57/utils';
...
async function main() {
  ...
  try {
    ...
    if (!lnurlMetadata || !lnurlMetadata.allowsNostr || !lnurlMetadata.nostrPubkey) {
      throw new Error('Failed to fetch valid LNURL metadata or the service does not support zaps');
    }
    
    const lnurlServerPubkey = lnurlMetadata.nostrPubkey;
    console.log(`✅ LNURL server supports zaps with pubkey: ${lnurlServerPubkey.slice(0, 8)}...\n`);
    ...
    console.log(`✅ Created zap request with ID: ${aliceZapRequest.id.slice(0, 8)}...\n`);
    ...
    try {
      ...
      console.log('In a real app, you would pay this invoice with a Lightning wallet');
      console.log('Since this is a signet address, no real payment is required.\n');
      ...
      const splitAmounts = zapClient.calculateZapSplitAmounts(totalZapAmount, splitInfo);
      
      console.log(`\nIf Charlie's note receives a ${totalZapAmount / 1000} sat zap, it will be split as:`);
      for (const amount of splitAmounts) {
        console.log(`  💰 ${amount.pubkey.slice(0, 8)}... gets ${amount.amount / 1000} sats via ${amount.relay}`);
      }
      
      // Validate the simulated zap receipt
      console.log('\nValidating the simulated zap receipt:');
      const validationResult = validateZapReceipt(simZapReceipt, lnurlServerPubkey);
      console.log(`Validation result: ${JSON.stringify(validationResult, null, 2)}`);
      console.log('\nNote: In a real application, the LNURL server would create and sign the zap receipt');
      console.log('The validation would fail for our simulated receipt because we cannot properly sign it');
