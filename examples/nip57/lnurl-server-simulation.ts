...
class MockLightningWallet {
  ...
  generateInvoice(amountMsat: number, description: string): { 
    bolt11: string, 
    paymentHash: string 
  } {
...
class MockLnurlServer {
  ...
  private pendingZapRequests: Map<string, { 
  ...
  private async handlePayment(paymentHash: string) {
    try {
      ...
      } as UnsignedEvent, this.keypair.privateKey);
...
async function main() {
  ...
  const noteEvent = await recipientClient.publishTextNote('This is a test note. Send me sats!');
  ...
  console.log(`✅ LNURL server supports zaps with pubkey: ${lnurlPayResponse.nostrPubkey.slice(0, 8)}...\n`);
  ...
  const zapRequestTemplate = createZapRequest(zapRequestOptions, senderKeypair.publicKey);
  ...
  callbackUrl.searchParams.append('nostr', encodeURIComponent(JSON.stringify(signedZapRequest)));
  ...
  console.log('(The mock server will automatically simulate payment in 1 second)\n');
