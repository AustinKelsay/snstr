...
async function main() {
  ...
  const noteEvent = await client.publishTextNote(
    "This is a note that will receive zaps!",
  );
  ...
  if (relaysTag) {
    ...
    console.log(
      'This format follows NIP-57 specification: a single tag with "relays" as the first element\n',
    );
  }
  ...
  const multiRelaysTag = signedMultiRelayRequest.tags.find(
    (tag) => tag[0] === "relays",
  );
  if (multiRelaysTag) {
  ...
  console.log(
    `🧾 Created zap receipt with ID: ${signedZapReceipt.id.slice(0, 8)}...\n`,
  );
  ...
  const subId = client.subscribe(
    ...
    (event, relayUrl) => {
      ...
      try {
        ...
        console.log(
          `   Amount: ${zapRequest.tags.find((tag: string[]) => tag[0] === "amount")?.[1] || "unknown"} millisats`,
        );
        console.log(`   Sender: ${zapRequest.pubkey.slice(0, 8)}...`);
        console.log(`   Recipient: ${pTag[1].slice(0, 8)}...`);
        console.log(
          `   Event ID: ${event.tags.find((tag: string[]) => tag[0] === "e")?.[1]?.slice(0, 8)}...`,
        );
        ...
        console.log(
          "   In a real implementation, the hash of the zap request would be verified",
        );
        console.log("   against the description hash in the bolt11 invoice.");
      } catch (error) {
        console.error(
          `❌ Error processing zap receipt: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    },
  );
  ...
  console.log("----------------------------------------------");
  console.log(
    "The bolt11 invoice description_hash field must match the SHA-256 hash of the zap request.",
  );
  console.log(
    "This ensures the invoice was actually generated for the specific zap request.",
  );
  console.log(
    "Without this verification, malicious LNURL servers could generate fake zap receipts.",
  );
  ...
  const tamperedReceiptTemplate = {
    ...
    tags: [
      ...
      [
        "bolt11",
        "",
      ],
  ...
  const tamperedReceipt = await createSignedEvent(
    tamperedReceiptTemplate,
    lnurlServerKeypair.privateKey,
  );
  console.log(
    `Created tampered receipt with ID: ${tamperedReceipt.id.slice(0, 8)}...`,
  );
  ...
  console.log(
    "✓ Description hash verification would detect the tampered receipt",
  );
  ...
  const signedNoteWithSplit = await createSignedEvent(
    noteWithSplitTemplate,
  ...
  splitInfo.forEach((info) => {
    console.log(
      `   Pubkey: ${info.pubkey.slice(0, 8)}..., Relay: ${info.relay}, Weight: ${info.weight}`,
    );
  });
  ...
  splitAmounts.forEach((info) => {
    console.log(
      `   Pubkey: ${info.pubkey.slice(0, 8)}...: ${info.amount / 1000} sats`,
    );
  });
  ...
  setTimeout(async () => {
    try {
      ...
      await relay.close();
      console.log("Cleanup completed successfully."); // Force exit successfully
      process.exit(0);
    } catch (err) {
      console.error("Cleanup failed. Error:", err); // Keep a general error message
