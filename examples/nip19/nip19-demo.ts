...
function demonstrateTLVEncoding() {
  ...
  try {
    ...
    console.log(
      `Kind: ${decoded.kind !== undefined ? decoded.kind : "(Not part of nevent encoding)"}`,
    );
    console.log("Relays:");
    if (decoded.relays && decoded.relays.length > 0) {
      decoded.relays.forEach((relay) => console.log(`- ${relay}`));
...
function demonstrateErrorHandling() {
  ...
  const invalidPubkey =
    "3bf0c63fcb93463407af97a5e5ee64fa883d107ef9e558472c4eb9aaaefa45"; // too short
...
function demonstrateRealWorldUsage() {
  ...
  try {
    ...
    console.error(
      `Error creating shareable profile: ${(error as Error).message}`,
    );
  }
    ...
    try {
      ...
      switch (decoded.type) {
        ...
        case "nprofile": {
          ...
          console.log(
            `Suggested relays: ${profileData.relays?.join(", ") || "None"}`,
          );
          console.log(
            "Action: Navigate to user profile and connect to suggested relays",
          );
          break;
        }
        ...
        default:
          console.log(
            `Entity type ${decoded.type} not handled in this example`,
          );
      }
    } catch (error) {
