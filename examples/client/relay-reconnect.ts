...
relay.on(RelayEvent.Disconnect, (url) => {
  console.log(`❌ Disconnected from ${url}, will attempt to reconnect automatically`);
});
  ...
  .then(success => {
    if (success) {
      ...
      const subId = relay.subscribe(
        ...
        (event) => {
          if (verbose || debug) {
            console.log(`Received event: ${event.kind} from ${event.pubkey.slice(0, 8)}...`);
          } else {
...
console.log('- relay.setMaxReconnectAttempts(10) - Set maximum reconnection attempts');
console.log('- relay.setMaxReconnectDelay(30000) - Set maximum delay between attempts (ms)'); 