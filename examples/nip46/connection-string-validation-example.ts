...
import { parseConnectionString, buildConnectionString } from "../../src/nip46/utils/connection";
...
const validLowercasePubkey = "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef";
// Invalid uppercase pubkey (should be rejected)
const invalidUppercasePubkey = "1234567890ABCDEF1234567890ABCDEF1234567890ABCDEF1234567890ABCDEF";
const validRelay = "wss://relay.example.com";

console.log("1. Valid connection string parsing:");
console.log("   ✓ Connection strings with properly formatted lowercase pubkeys are accepted\n");
...
console.log("   ✗ Connection strings with uppercase pubkeys are properly rejected\n");
...
try {
  ...
  console.log("   → ERROR: Should have been rejected but was accepted:", result);
} catch (error) {
...
console.log("   The fix ensures that pubkey case is preserved during validation\n");

// Before the fix, URL.hostname would have lowercased this automatically
const mixedCasePubkey = "1234567890ABCdef1234567890ABCdef1234567890ABCdef1234567890ABCdef";
...
console.log("   Testing that valid pubkeys work through the full build/parse cycle\n");
...
try {
  ...
  console.log("     • Pubkey matches:", parsedResult.pubkey === connectionOptions.pubkey);
  console.log("     • Relays match:", JSON.stringify(parsedResult.relays) === JSON.stringify(connectionOptions.relays));
  console.log("     • Secret matches:", parsedResult.secret === connectionOptions.secret);
  console.log();
} catch (error) {
...
const edgeCases = [
  ...
  },
  {
    ...
    connectionString: "bunker://1234567890abcdefg234567890abcdef1234567890abcdef1234567890abcdef?relay=wss://relay.com",
    shouldFail: true
  },
  {
    name: "Empty pubkey",
...
edgeCases.forEach(({ name, connectionString, shouldFail }) => {
  try {
    ...
    if (shouldFail) {
      console.log(`   → ${name}: ERROR - Should have failed but parsed successfully`);
    } else {
      console.log(`   → ${name}: ✓ Parsed successfully (pubkey: ${result.pubkey.substring(0, 16)}...)`);
    }
  } catch (error) {
    if (shouldFail) {
