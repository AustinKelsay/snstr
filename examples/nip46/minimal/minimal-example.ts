...
class MinimalNIP46Client {
  ...
  private async sendRequest(method: string, params: string[]): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const encrypted = encryptNIP04(json, this.clientKeys.privateKey, this.signerPubkey);
  ...
  private handleMessage(data: string): void {
    try {
      ...
      const pTag = event.tags.find((tag: string[]) => tag[0] === 'p' && tag[1] === this.clientKeys.publicKey);
      if (!pTag) return;
...
class MinimalNIP46Bunker {
  ...
  private async handleMessage(data: string): Promise<void> {
    try {
      ...
      const pTag = event.tags.find((tag: string[]) => tag[0] === 'p' && tag[1] === this.signerKeys.publicKey);
      if (!pTag) return;
  ...
  private async handleRequest(request: any, clientPubkey: string): Promise<any> {
      ...
      switch (method) {
        ...
        case 'sign_event':
          ...
          try {
            ...
            // Sign the event
            eventData.sig = await signEvent(eventData.id, this.userKeys.privateKey);
  ...
  private async sendResponse(clientPubkey: string, response: any): Promise<void> {
    try {
