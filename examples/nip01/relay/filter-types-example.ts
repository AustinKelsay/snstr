...
async function main() {
  ...
  await runFilterExample(
    client,
    referenceFilter,
    "Events with reference to NIP-01",
  );
  ...
  await runFilterExample(
    client,
    dTagFilter,
    "Addressable events with d-tag=my-first-article",
  );
  ...
  await runFilterExample(
    client,
    combinedFilter,
    "Events with #t=nostr, kind=30023, by our author",
  );
  ...
  await runFilterExample(
    client,
    customFilter,
    "Events with title=Understanding Nostr",
  );
...
async function runFilterExample(
  client: Nostr,
  filter: NostrFilter,
  description: string,
): Promise<void> {
      ...
      () => {
        ...
        if (events.length > 0) {
          ...
          events.forEach((event) => {
            console.log(
              `- Kind ${event.kind}, Content: "${event.content.substring(0, 40)}${event.content.length > 40 ? "..." : ""}"`,
            );
            console.log(
              `  Tags: ${JSON.stringify(event.tags.slice(0, 2))}${event.tags.length > 2 ? " ..." : ""}`,
            );
          });
        } else {
