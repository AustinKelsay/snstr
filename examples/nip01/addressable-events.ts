...
async function main() {
  try {
    ...
    const articleEvent = createAddressableEvent(
      ...
      [["title", "My First Article"], ["published_at", new Date().toISOString()]] // Additional tags
    );
    
    // Sign and publish the event
    console.log("Publishing initial article...");
    const signedArticleEvent = await createSignedEvent(articleEvent, keys.privateKey);
    ...
    const updatedContent = "This is the updated version of my article with more content and edits";
    ...
    const signedUpdatedArticleEvent = await createSignedEvent(updatedArticleEvent, keys.privateKey);
    ...
    if (latestArticle) {
      ...
      console.log(`Created at: ${new Date(latestArticle.created_at * 1000).toLocaleString()}`);
    ...
    const article2Content = "This is my second article with a different identifier";
    ...
    const article2Event = createAddressableEvent(
      ...
      article2Content,
      keys.privateKey,
      [["title", "My Second Article"], ["published_at", new Date().toISOString()]]
    );
    
    console.log("Publishing second article...");
    const signedArticle2Event = await createSignedEvent(article2Event, keys.privateKey);
    ...
    const signedProductEvent = await createSignedEvent(productEvent, keys.privateKey);
    ...
    allProducts.forEach((product, index) => {
      try {
        ...
        console.log(`Product ${index + 1}: ${productData.name} - $${productData.price} ${productData.currency}`);
      } catch (e) {
    ...
    try {
      ...
      console.log("✅ Correctly caught error when creating invalid addressable event:");
