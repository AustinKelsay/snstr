...
async function runVectors() {
  ...
  console.log(`Conversation key derivation: ${passedCount}/${Math.min(5, conversationVectors.length)} tests passed\n`);
  ...
  if (testVectors.v2.valid.get_message_keys) {
    ...
    for (let i = 0; i < Math.min(3, keyDerivations.length); i++) {
      ...
      try {
        ...
        const derivedChachaNonce = Buffer.from(keys.chacha_nonce).toString("hex");
        ...
        if (keysMatch) {
          ...
          if (derivedChachaKey !== chacha_key) {
            console.log(`  Expected chacha_key: ${chacha_key.substring(0, 10)}...`);
            console.log(`  Got chacha_key:      ${derivedChachaKey.substring(0, 10)}...`);
          }
          ...
          if (derivedHmacKey !== hmac_key) {
            ...
            console.log(`  Got hmac_key:      ${derivedHmacKey.substring(0, 10)}...`);
          }
        }
      } catch (error) {
    ...
    console.log(`Message key derivation: ${messageKeysPassed}/${Math.min(3, keyDerivations.length)} tests passed\n`);
  } else {
  ...
  if (testVectors.v2.valid.encrypt_decrypt) {
      ...
      try {
        ...
        if (payload) {
          ...
          if (decrypted === plaintext) {
            ...
            console.log(`Vector ${i + 1}: Decryption successful - "${plaintext}"`);
          } else {
        ...
        if (ourDecrypted === plaintext) {
          console.log(`Vector ${i + 1}: Our encryption/decryption roundtrip successful`);
        } else {
    ...
    console.log(`Encryption/decryption: ${encryptDecryptPassed}/${Math.min(3, encryptDecryptVectors.length)} tests passed\n`);
  } else {
  ...
  for (const version of [0, 1, 2]) {
    try {
      ...
      if (versionMatches && decryptionWorks) {
        console.log(`Version ${version}: Encryption and decryption successful ✅`);
      } else {
        console.log(`Version ${version}: Test failed ❌`);
        if (!versionMatches) {
          console.log(`  - Version mismatch: expected ${version}, got ${decoded.version}`);
        }
        if (!decryptionWorks) {
          console.log(`  - Decryption failed: expected "${simpleVector.plaintext}", got "${decrypted}"`);
        }
      }
    } catch (error) {
  ...
  if (specificVector) {
    ...
    if (sec1 && sec2 && plaintext && payload) {
      ...
      console.log(`Shared secret: ${Buffer.from(sharedSecret).toString("hex").substring(0, 20)}...`);
      
      // Decode payload
      const decodedPayload = decodePayload(payload);
      console.log(`\nDecoded payload:`);
      console.log(`- Version: ${decodedPayload.version}`);
      console.log(`- Nonce: ${Buffer.from(decodedPayload.nonce).toString("hex").substring(0, 20)}...`);
      console.log(`- Ciphertext length: ${decodedPayload.ciphertext.length}`);
      console.log(`- MAC: ${Buffer.from(decodedPayload.mac).toString("hex").substring(0, 20)}...`);
