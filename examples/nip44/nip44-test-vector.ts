...
async function debug() {
  ...
  console.log(
    `Matches expected:   ${sharedSecret1Hex === vector.conversation_key}`,
  );
  ...
  console.log(
    `Payload MAC:    ${Buffer.from(decodedPayload.mac).toString("hex")}`,
  );
  console.log(
    `MACs match:     ${Buffer.from(calculatedMac).toString("hex") === Buffer.from(decodedPayload.mac).toString("hex")}`,
  );
  ...
  try {
    ...
    console.log(
      `Matches expected plaintext: ${decrypted === vector.plaintext}`,
    );
  } catch (error) {
    console.error(
      "Decryption failed:",
