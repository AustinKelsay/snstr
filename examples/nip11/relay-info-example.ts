...
async function main() {
  ...
  for (const relay of relays) {
    ...
    try {
      ...
      if (info) {
        ...
        console.log(`Supports NIPs 1 and 4: ${supportsNIP1and4 ? 'Yes' : 'No'}`);
  ...
  try {
    ...
    console.log(`Fetched fresh data from relay.damus.io: ${info ? 'Success' : 'Failed'}`);
  } catch (error) {
...
function printRelayInfo(url: string, info: RelayInfo) {
  ...
  if (info.limitation) {
    ...
    console.log(`  Payments required: ${info.limitation.payments_required ? 'Yes' : 'No'}`);
    console.log(`  Auth required: ${info.limitation.auth_required ? 'Yes' : 'No'}`);
  }
  ...
  if (info.fees) {
    ...
    if (info.fees.admission) {
      ...
      info.fees.admission.forEach(fee => {
        console.log(`    ${fee.amount} ${fee.unit}${fee.period ? ` per ${fee.period}` : ''} - ${fee.description || ''}`);
      });
    }
