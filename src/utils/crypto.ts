 ...
 */
export async function signEvent(eventId: string, privateKey: string): Promise<string> {
 ...
 */
export async function verifySignature(eventId: string, signature: string, publicKey: string): Promise<boolean> {
  try {
    const eventIdBytes = hexToBytes(eventId);
