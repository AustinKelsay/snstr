...
export function validateTags(tags: unknown): string[][] {
  ...
  return tags.map((tag, tagIndex) => {
    ...
    return tag.map((element, _elementIndex) => {
      const elementStr = sanitizeString(
        element,
...
export function validateFilter(filter: unknown): Filter {
  ...
  if (f.authors !== undefined) {
    ...
    validatedFilter.authors = f.authors.map((author, index) => {
      const authorStr = sanitizeString(
        author,
  ...
  if (f.limit !== undefined) {
    validatedFilter.limit = validateNumber(
      f.limit,
      SECURITY_LIMITS.MIN_LIMIT,
      SECURITY_LIMITS.MAX_LIMIT,
      "filter.limit",
    );
  }
  ...
  if (f.since !== undefined) {
    validatedFilter.since = validateNumber(
      f.since,
      SECURITY_LIMITS.MIN_SINCE,
      SECURITY_LIMITS.MAX_SINCE,
      "filter.since",
    );
  }
  ...
  if (f.until !== undefined) {
    validatedFilter.until = validateNumber(
      f.until,
      SECURITY_LIMITS.MIN_UNTIL,
      SECURITY_LIMITS.MAX_UNTIL,
      "filter.until",
    );
  }
  ...
  if (
    validatedFilter.since !== undefined &&
  ...
  if (f.search !== undefined) {
    validatedFilter.search = sanitizeString(
      f.search,
      SECURITY_LIMITS.MAX_SEARCH_LENGTH,
    );
  }
  ...
  for (const [key, value] of Object.entries(f)) {
    if (key.startsWith("#") && key.length === 2) {
      ...
      (validatedFilter as Record<string, unknown>)[key] = value.map(
        (tagValue, _index) => {
