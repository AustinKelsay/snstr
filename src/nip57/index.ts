 ...
 */
export function createZapRequest(options: ZapRequestOptions, signerPubkey: string): EventTemplate {
  const tags: string[][] = [
 ...
 * @returns Unsigned event template
 */
export function createZapReceipt(options: ZapReceiptOptions, signerPubkey: string): EventTemplate {
  const tags: string[][] = [
    ['p', options.recipientPubkey]
  ];
 ...
 */
export function validateZapReceipt(zapReceipt: NostrEvent, lnurlPubkey: string): ZapValidationResult {
  // Check basics
  if (!zapReceipt) {
  ...
  if (zapReceipt.kind !== ZAP_RECEIPT_KIND) {
    return { valid: false, message: `Not a zap receipt event: kind ${zapReceipt.kind} instead of ${ZAP_RECEIPT_KIND}` };
  }
  
  if (zapReceipt.pubkey !== lnurlPubkey) {
    return { valid: false, message: `Zap receipt not from expected LNURL provider: ${zapReceipt.pubkey} vs expected ${lnurlPubkey}` };
  }
  ...
  try {
    ...
    return { valid: false, message: `Invalid zap request description: ${e instanceof Error ? e.message : String(e)}` };
  }
  ...
  try {
    ...
    return { valid: false, message: `Error verifying signature: ${error instanceof Error ? error.message : String(error)}` };
  }
  
  // Validate zap request
  if (zapRequest.kind !== ZAP_REQUEST_KIND) {
    return { valid: false, message: `Not a zap request event: kind ${zapRequest.kind} instead of ${ZAP_REQUEST_KIND}` };
  }
  ...
  // Validate recipient
  const zapRequestPTag = zapRequest.tags.find((tag: string[]) => tag[0] === 'p');
  if (!zapRequestPTag) {
    return { valid: false, message: 'Missing p tag in zap request' };
  }
  
  if (zapRequestPTag[1] !== pTag[1]) {
    return { valid: false, message: `Recipient mismatch: ${zapRequestPTag[1]} vs ${pTag[1]}` };
  }
  
  // Validate event ID if present
  const zapRequestETag = zapRequest.tags.find((tag: string[]) => tag[0] === 'e');
 ...
 */
export function parseZapSplit(event: NostrEvent): { pubkey: string, relay: string, weight: number }[] {
  ...
  const hasWeights = splitInfo.some((info: { weight: number }) => info.weight > 0);
  if (!hasWeights) {
    const equalWeight = 1;
    return splitInfo.map((info: { pubkey: string, relay: string }) => ({ ...info, weight: equalWeight }));
  }
