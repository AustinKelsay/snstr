...
export class NostrWalletService {
  ...
  private async handleEvent(event: NostrEvent): Promise<void> {
    ...
    try {
      ...
      console.log(`Handling request from ${event.pubkey}. Event ID: ${event.id}`);
      ...
      try {
        ...
        console.log('Successfully decrypted content: ', decrypted.substring(0, 50) + '...');
      } catch (decryptError) {
   ...
   */
  private async sendResponse(clientPubkey: string, response: NIP47Response, requestId: string): Promise<void> {
    // Create event template
    const eventTemplate = {
    ...
    try {
      ...
      switch (request.method) {
        ...
        case NIP47Method.SIGN_MESSAGE:
          result = await this.walletImpl.signMessage?.(
